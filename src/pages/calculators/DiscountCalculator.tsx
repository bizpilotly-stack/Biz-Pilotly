...
export const DiscountCalculator: React.FC = () => {
  ...
  const relatedCalculators = calculatorService.getRelatedCalculators('discount');
  const jsonLd = calculatorService.getJsonLd('discount');
      ...
      <div className="container">
        ...
        <div className="calc-detail-layout">
          <div className="calc-inputs-panel">
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="originalPriceInput">Original Standard Fee / Price ($)</label>
              ...
              <p className="form-hint">Standard rate before promotional or client reduction</p>
            </div>

            <div className="form-group">
              <label className="form-label" htmlFor="discountPercentInput">Discount Percentage (%)</label>
          ...
          <div className="calc-results-panel">
            <div className="calc-result-header">
              <div style={{ fontSize: '0.75rem', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 700 }}>
                Final Discounted Invoiced Price
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.5rem', fontSize: '0.875rem', color: '#38bdf8' }}>
                <Sparkles size={16} />
                <span>Client saves <strong>{formatCurrency(res.discountAmount)}</strong> ({formatPercent(res.discountPercent)})</span>
              </div>
            </div>

            <div className="calc-stats-grid">
              <div className="calc-stat-item">
                <div className="calc-stat-label">Total Savings</div>
                <div className="calc-stat-val" style={{ color: '#34d399' }}>{formatCurrency(res.discountAmount)}</div>
              </div>

              <div className="calc-stat-item">
                <div className="calc-stat-label">Discount Rate</div>
                <div className="calc-stat-val">-{formatPercent(res.discountPercent)}</div>
              </div>
              ...
              <div className="calc-stat-item">
                <div className="calc-stat-label">Net Received</div>
                <div className="calc-stat-val">{formatCurrency(res.finalPrice)}</div>
              </div>
            </div>
            ...
            <div style={{ marginTop: '2rem', paddingTop: '1.5rem', borderTop: '1px solid rgba(255, 255, 255, 0.1)' }}>
              <Link to={`${docsBase}/invoice`} className="btn btn-gold" style={{ width: '100%', justifyContent: 'center' }}>
                <FileText size={16} />
                <span>Create Invoice for {formatCurrency(res.finalPrice)}</span>
                <ArrowRight size={16} />
              </Link>
            </div>
          </div>
        ...
        <div className="calc-cta-banner">
          <div className="calc-cta-text">
            ...
            <p>Generate an official invoice reflecting applied discounts and total savings.</p>
          </div>
